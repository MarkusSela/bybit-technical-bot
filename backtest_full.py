...
def simulate(df_15m, df_daily):
    ...
    for i in range(CANDLES_LOOKBACK, len(df_15m)):
        ...
        if df_daily is not None:
            ...
            if daily_trend == 'bullish' and direction_15m == 'SHORT':
                continue
            if daily_trend == 'bearish' and direction_15m == 'LONG':
                continue
...
print('=' * 70)
print('BACKTEST TOP 25 | Balance $' + str(STARTING_BALANCE) + ' | MIN_VALUE $' + str(MIN_VALUE))
print('Fee incluse | SL/TP dinamici per score | Filtro daily ON | 180 giorni')
print('=' * 70)
...
for pair in PAIRS:
    ...
    trades, blocked = simulate(df_15m, df_daily)
...
print()
print('TOP 6 SUGGERITI (solo positivi):')
for r in positivi[:6]:
    print('  ' + r['pair'] + ' | WR: ' + str(r['wr']) + '% | PnL: ' + str(r['pnl']) + '%')

print()
tot_trades = sum(r['trades'] for r in positivi[:6])
tot_pnl    = round(sum(r['pnl'] for r in positivi[:6]), 1)
tot_wr     = round(sum(r['wr'] * r['trades'] for r in positivi[:6]) / tot_trades, 1) if tot_trades > 0 else 0
tot_blocked = sum(r['blocked'] for r in results)

print('TOP 6 COMBINATO | Trade: ' + str(tot_trades) + ' | WR medio: ' + str(tot_wr) + '% | PnL: +' + str(tot_pnl) + '%')
print()
print('Trade totali bloccati da MIN_VALUE $' + str(MIN_VALUE) + ': ' + str(tot_blocked))
print('=' * 70)
